...
# Compatibility helper: try multiple possible column names
def get_col(*names):
    for name in names:
        if name in df.columns:
            return pd.to_numeric(df[name], errors="coerce").to_numpy().ravel()
    raise ValueError(f"Missing column(s): {names}")


# Positions: prefer pose1_/pose2_, else actual_pos_/target_pos_
p1x, p1y, p1z = (
...
if "time" in df.columns:
    ...
    if not {"sample_stamp_sec", "sample_stamp_nanosec"}.issubset(df.columns):
        raise ValueError(
            "Need either 'time' column or both 'sample_stamp_sec' and 'sample_stamp_nanosec'."
        )
    sec = (
        pd.to_numeric(df["sample_stamp_sec"], errors="coerce").fillna(0).astype("int64")
    )
    nsec = (
        pd.to_numeric(df["sample_stamp_nanosec"], errors="coerce")
        .fillna(0)
        .astype("int64")
    )
...
(p1_scatter,) = ax.plot(
    [p1x[0]], [p1y[0]], [p1z[0]], marker="o", linestyle="", label="pose1"
)
(p2_scatter,) = ax.plot(
    [p2x[0]], [p2y[0]], [p2z[0]], marker="o", linestyle="", label="pose2"
)
...
def set_frame(i):
    ...
    p1_scatter.set_data([p1x[i]], [p1y[i]])
    p1_scatter.set_3d_properties([p1z[i]])
    p2_scatter.set_data([p2x[i]], [p2y[i]])
    p2_scatter.set_3d_properties([p2z[i]])
    p1_trail.set_data(p1x[: i + 1], p1y[: i + 1])
    p1_trail.set_3d_properties(p1z[: i + 1])
    p2_trail.set_data(p2x[: i + 1], p2y[: i + 1])
    p2_trail.set_3d_properties(p2z[: i + 1])
