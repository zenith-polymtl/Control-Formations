...
def build_soft_trajectory(t):
    ...
    def mix_sin(a, w, ph):
        return (a[None, :] * np.sin(w[None, :] * t[:, None] + ph[None, :])).sum(axis=1)

    def mix_cos_w(a, w, ph):
        return (
            a[None, :] * w[None, :] * np.cos(w[None, :] * t[:, None] + ph[None, :])
        ).sum(axis=1)

    def mix_sin_w2(a, w, ph):
        return (
            a[None, :] * w[None, :] ** 2 * np.sin(w[None, :] * t[:, None] + ph[None, :])
        ).sum(axis=1)

    # Position / velocity / acceleration (analytic)
    x = S0 * mix_sin(Ax, wx, phx)
    y = S0 * mix_sin(Ay, wy, phy)
    z = Z0 + S0 * mix_sin(Az, wz, phz)
    vx = S0 * mix_cos_w(Ax, wx, phx)
    vy = S0 * mix_cos_w(Ay, wy, phy)
    vz = S0 * mix_cos_w(Az, wz, phz)
    ax = -S0 * mix_sin_w2(Ax, wx, phx)
    ay = -S0 * mix_sin_w2(Ay, wy, phy)
    az = -S0 * mix_sin_w2(Az, wz, phz)
...
class Ballon_publisher(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            String, "/arrival", self.callback, 10
        )
    ...
    def callback(self, msg: String):
        self.get_logger().info(
            f"Arrival of : {msg.data}, starting ballon run, scoring starting in 5 s"
        )
    ...
    def ballon_publisher(self):
        ...
        if time.time() - self.start_time < T:
            ...
            msg.pose.position.x = (
                float(self.x)
                + float(self.begin_position[1])
                - float(self.initial_balloon_computed[0])
            )
            msg.pose.position.y = (
                float(self.y)
                + float(self.begin_position[0])
                - float(self.initial_balloon_computed[1])
            )
            msg.pose.position.z = (
                float(self.z)
                - float(self.begin_position[2])
                - float(self.initial_balloon_computed[2])
            )
