...
Dans votre home Ubuntu (pas dans `/mnt/c`), créer un dossier `demo-docker` avec trois fichiers.
...
# Librairie.s Python de l'équipe. --no-cache-dir : pip ne garde pas les archives
...
**3. Bâtir l'image.** Depuis le dossier `B2` de la formation, qui doit être dans votre home WSL et non dans `/mnt/c` :
...
**8. Lire le message de statut, dans un deuxième terminal.** Ouvrir un nouveau terminal WSL, retourner dans `B2`, et entrer dans le même conteneur :
...
**Le Makefile de la formation.** Le fichier [Makefile](Makefile) est dans le dossier `B2`. Il fait en une commande tout le tour de la section 2.7 :
...
**S'en servir.** Avec la simulation MP démarrée, depuis le dossier `B2` :
...
**Deux limites assumées.** `Ctrl+C` peut couper make avant la ligne de nettoyage : dans ce cas, `make down` finit le travail, et c'est pour ça que la cible existe. Et comme `up` relance mavros à chaque appel, ouvrir deux `make shell` sans `make down` entre les deux laisse deux mavros se battre pour le même port.
...
4. **Bonus.** Ajouter une cible `listen-position` qui écoute `/mavros/local_position/pose`, et la rouler dans un deuxième terminal pendant que la mission roule. Les coordonnées qui défilent et le drone qui bouge sur la carte de MP sont la même information, prise à deux endroits différents de la chaîne. Attention, ne marchera pas sans requête du message de position, c'est comme ça que mavlink marche. TODO : Ajouter requête de position local_position. 
